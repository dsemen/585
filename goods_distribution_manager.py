...
stock_dict = {"Обручальные кольца": 0, 
              "Буквы и Зодиаки с описанием - 3": 1,
              "Товы": 2, 
...
def stockfunc():
    ...
    if save_name == "Буквы и Зодиаки (только подвесы)":
...
distr_lbl = Label(distr, text='Магазины для распределения, через запятую (не требуется для букв и зодиаков)')
distr_lbl.pack(pady=7)

distr_t = Text(distr, height=6)
distr_t.pack()# поле для магазинов
...
def distr_all(lst):
    ...
    stock = stock[stock['КодСклада'].isin(lst) & stock['ТоварНо'].isin(tov_list)]
    ...
    pivot = pd.DataFrame({'КодСклада':lst}).merge(pivot, how='left')
    ...
    add_zeroes = [list(np.zeros(len(lst), dtype='int'))] * len(add_cols)
...
def distr_obr():
    shops = pd.DataFrame({'номер': shop_list}, dtype='int')
...
def distrfunc():
    ...
    shop_str = distr_t.get(1.0, 'end-1c')
    shop_lst = [int(x) for x in shop_str.split(',')]
...
def demand():
    ...
    stock = pd.read_excel(r'C:\Остатки\обручи.xlsx', usecols=['КодСклада','Товарная подгруппа','Размер'])
...
def cutbycolumn():
    path = 'C:/рушники/' + pr_txt2.get(1.0, "end-1c") + '.xlsx'
    df = pd.read_excel(path)
    column = varpr_cmb.get()
    lst = df[column].unique().tolist()
    [df[df[column] == x].to_excel(path[:11] + str(x) + path[15:], sheet_name='Движение товара', index=False) for x in lst]
